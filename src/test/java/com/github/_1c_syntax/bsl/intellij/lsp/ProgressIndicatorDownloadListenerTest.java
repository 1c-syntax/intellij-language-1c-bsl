...
/**
 * Тесты отображения прогресса скачивания на индикаторе IntelliJ: троттлинг перерисовки, доля/текст
 * при известном размере, неопределённый режим при неизвестном и проброс отмены задачи.
 */
public class ProgressIndicatorDownloadListenerTest extends BasePlatformTestCase {

  public void testReportsFractionAndThrottlesWithinPercent() {
    var total = 200L * 1024 * 1024; // 200 МБ, 1% == 2 МБ
    var indicator = mock(ProgressIndicator.class);
    var listener = new ProgressIndicatorDownloadListener(indicator);
  ...
  public void testIsIndeterminateWhenTotalUnknown() {
    var indicator = mock(ProgressIndicator.class);
    var listener = new ProgressIndicatorDownloadListener(indicator);
